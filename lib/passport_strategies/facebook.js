...
// TODO: MODULARIZE THIS AWAY INTO A FILE THAT TAKES AN OBJECT WITH FB-SPECIFIC PARAMS
function handleFacebookResponse(req, accessToken, refreshToken, profile, done) {
  ...
  User.findOne({ 'auth.facebook.facebookId': profile.id }, function(err, user) {
    ...
    if (!user || !user.auth.facebook.facebookId) {
      ...
      // No user? make one.
      getFacebookInfo(accessToken, null, function(err, fbData) {
        if(err) {
          return done('error accessing facebook login');
        }
      ...
      // Update access token
      user.auth.facebook.facebookAccessToken = accessToken;
      user.save();
