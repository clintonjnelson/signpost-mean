...
module.exports = function(signBuilder) {
  ...
  function buildFacebook(fbData) {
    // base attrs:   customBgColor, description, knownAs, linkUrl, published
    // custom attrs: bgColor, email, facebookId, facebookPicUrl, icon, signType
    // NOTE: most not needed OR covered by defaults

    var signProps = {
      // ----------------- BASE -----------------------
      // description:    fbData.description,                // optional for updates?
      knownAs:        (fbData.name || fbData.knownAs),
      linkUrl:        (fbData.link || fbData.linkUrl),
      // ----------------- CUSTOM ---------------------
      email:          fbData.email,
    ...
    // load schema data into new facebook sign object
